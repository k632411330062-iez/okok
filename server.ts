    ...
    for (const model of models) {
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          ...
          // Check for retriable errors (429: Rate Limit, 503: Service Unavailable, 504: Gateway Timeout)
          ...
          if (isRateLimit || isBusy || isTimeout) {
            ...
            const errorType = isRateLimit ? "Rate Limit (429)" : isBusy ? "Service Busy (503)" : "Timeout";
            console.warn(`[AI API] ${errorType} for ${model}. Retrying in ${Math.round(delay)}ms...`);
            
            await new Promise(resolve => setTimeout(resolve, delay));
          } else {
            // Non-retriable error for this model (e.g., 400 Bad Request, 403 Forbidden)
            console.error(`[AI API] Non-retriable error for ${model}: ${error.message}`);
            break; // Try next model in the list
          }
        }
      }
      console.log(`[AI API] Exhausted retries for ${model}, switching to next model if available.`);
    }
    
    console.error("[AI API] All models and retries exhausted.");
    throw lastError;
  };
...
${JSON.stringify(analysisData)}`,
  ...
  app.get("/api/stock/:symbol/chart", async (req, res) => {
    ...
    try {
      ...
      if (data.s === "ok") {
        // take only the last day's data, which means the latter portion
        ...
        // simple heuristic: last 25 points or so forms the intraday curve (day ~ 20 bars of 15m)
